---
name: re-project-loop
description: Use when running a long-horizon reverse-engineering project that spans many sessions - maintains REVERSE.md task ledger, labels.csv address database, dead_ends.md, and per-session logs so work compounds instead of repeating. For a single binary or crackme solved in one sitting use crackme-workflow instead.
---
